...
*测试�?   K
*时间�?   2017.11.19
*备注�?   若要获取更短的中断，减小�?PIT_LDVAL(pitno) 的赋值即可。官方的参数设置非常多，有空可以看看。此处是逐飞
*举例�?   PIT_Config(PIT_CH1, 1, 1);  PIT_CH1开�?ms中断，优先级�?
**************************************************************************/
...
void PIT_Config(uint8_t PIT_CHX, uint32_t ms, uint8_t priority)
{
    ...
    NVIC_DisableIRQ(PIT_CH_IRQ_NO + PIT_CHX);  //禁能该中�?
    ...
    NVIC_EnableIRQ(PIT_CH_IRQ_NO + PIT_CHX);   //使能该中�?
}
...
//            sprintf(ch,"L_编码�?%5d ", Value_End_L); 
//        OLED_Show_StrAll(0,  26, ch, 1);
//        sprintf(ch,"R_编码�?%5d ", Value_End_R);
