...
uint8_t AnBT_DMP_MPU6050_DEV_CFG(void)		
{
	  ...
	  if (anbt_dmp_data[0]!=PRODUCT_WHOAMI) 
		{
			printf("\r\nI2C从机地址设置错误，请将AD0端口接地");
			printf("\r\nanbt_dmp_data:  %d",anbt_dmp_data[0]);
    ...
    if (anbt_dmp_rev)  //Congrats, these parts are better
		{
			...
			{
				printf("Unsupported version=1.0");  
        return 6;
			}
    } 
		else 
      ...
      if (!anbt_dmp_rev) 
			{
				printf("Product ID error"); 		//产品编号错误
//				return 5;    
      } 
			else if (anbt_dmp_rev == 4) 
			{
				printf("Half Sensitivity Part"); 
...
uint8_t AnBT_DMP_MPU6050_Init(void)	
{
	...
	printf("\r\n1.DMP设备初始化成功"); 		//完成初始化
	//
	if(!mpu_set_sensors(INV_XYZ_GYRO | INV_XYZ_ACCEL)) printf("\r\n2.DMP设置传感器成功");		//完成传感去设定
	else printf("\r\n2.DMP设置传感器错误");
	//
	if(!mpu_configure_fifo(INV_XYZ_GYRO | INV_XYZ_ACCEL)) printf("\r\n3.设置FIFO成功");		//完成fifo设置
	else printf("\r\n3.设置FIFO失败");
	//
	if(!mpu_set_sample_rate(DEFAULT_MPU_HZ))printf("\r\n4.DMP设置采样率成功");			//完成DMP采样率设置
	else printf("\r\n4.DMP设置采样率失败");
	//
	if(!dmp_load_motion_driver_firmware())	printf("\r\n5.DMP加载运动驱动固件成功");				//开启DMP功能
	else printf("\r\n5.DMP加载运动驱动固件失败");
	//
	if(!dmp_set_orientation(inv_orientation_matrix_to_scalar(gyro_orientation))) printf("\r\n6.DMP完成设置初始方向设定偏差");		//初始方向设定偏差
	else printf("\r\n6.DMP设置初始方向设定偏差出错");
	//
	if(!dmp_enable_feature(DMP_FEATURE_6X_LP_QUAT | DMP_FEATURE_TAP |
	DMP_FEATURE_ANDROID_ORIENT | DMP_FEATURE_SEND_RAW_ACCEL | DMP_FEATURE_SEND_CAL_GYRO |
	DMP_FEATURE_GYRO_CAL))		printf("\r\n7.DMP使能成功");		//DMP使能
	else printf("\r\n7.DMP使能出错");
	//
	if(!dmp_set_fifo_rate(DEFAULT_MPU_HZ)) printf("\r\n8.DMP设置FIFO速率成功");		//DMP  fifo速度
	else printf("\r\n8.DMP设置FIFO速率失败");
	//
	run_self_test();
	if(!mpu_set_dmp_state(1)) printf("\r\n9.Dmp启动成功");				//DMP成功开启
	else 
        {
          printf("\r\n9.Dmp启动失败");
//          gpio_init (PTA19, GPO,0);
