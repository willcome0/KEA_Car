...
void Motor_Init(void)
{
    ...
    ftm_pwm_init(ftm2,ftm_ch3,12500,0);//右轮 //正转			//PC3	FTM2_CH3
    ftm_pwm_init(ftm2,ftm_ch2,12500,0);							//PC2	FTM2_CH2
    Pin_Output_Config(Motor_R_EN_Port, Motor_R_EN_Pin, 0);
    
    ftm_pwm_init(ftm2,ftm_ch1,12500,0);//左轮 //正转			//PC0	FTM2_CH0
    ftm_pwm_init(ftm2,ftm_ch0,12500,0);//这边废了，跳线到PB5  1	//PC1	FTM2_CH1
...
void Motor_Set_Go_Speed(uint16_t L, uint16_t R)
{
    ...
    FTM_PWM_set_CnV(ftm2, ftm_ch1, R);
...
void Motor_Set_Back_Speed(uint16_t L, uint16_t R)
{
    ...
    FTM_PWM_set_CnV(ftm2, ftm_ch1, 1);
